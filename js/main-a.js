...
$(document).on("ready", function () {
    $("#currency-block span").click(function () {
		...
		// кнопка ОТКРЫТЬ ДЕПОЗИТ
			var curclass = $('#currency-block').attr( 'class' );
			if((curclass == 'usd') || (curclass == 'rub')){
				$('#profit-calculator .btn-dep').attr('id', 'usd-rub');
			}else {
				$('#profit-calculator .btn-dep').attr('id', 'btc-ltc-eth-dash');
			}
    });
});
...
$('document').ready(function(){
   $('ul.tabs2').delegate('li:not(.current)', 'click', function() {
	  $(this).addClass('current').siblings().removeClass('current').parents('div.tab-block2').find('div.box2').hide().eq($(this).index()).fadeIn(150);
	});
  $('ul.tabs2 li').click(function(ee){
	 ee.preventDefault();
  });
});